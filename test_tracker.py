...
def get_vr_flex():
    """Polls SteamVR for the right controller and maps buttons/triggers to flex percentages."""
    if not vr_enabled:
    ...
    if right_id is not None:
        ...
        if result:
            ...
            # --- THUMB (A, B/Menu, Touchpad/Stick) ---
            thumb_btn = "None"
            thumb_flex = 0.0
            stick_x = state.rAxis[0].x
            stick_y = state.rAxis[0].y
            
            # Priority check: A is lowest on the controller (most flex/inward), Stick is highest
            ...
            # --- GRIP BUTTON (Middle Finger Distal) ---
...
def draw_skeleton(vr_data, flex_data):
    ...
    # Map thumb buttons to different lateral angles to fake 3D movement
    if thumb_btn == "A":
        thumb_angle = -100  # Pulled inward toward palm
        thumb_f = thumb_raw_flex * 0.8
    elif thumb_btn == "B":
        thumb_angle = -120  # Middle
        thumb_f = thumb_raw_flex * 0.6
    elif thumb_btn == "Stick":
        # Base resting angle is -140. Map stick X (-1 to 1) to swing it left/right by 30 degrees.
        thumb_angle = -140 + (stick_x * 30)
        # Base flex is 0.4. Map stick Y (-1 to 1) to curl/extend the thumb.
        thumb_f = min(1.0, max(0.0, 0.4 - (stick_y * 0.4)))
        # Override flex to 1.0 if the stick is physically clicked down
        if thumb_raw_flex == 1.0:
            thumb_f = 1.0
    else:
        thumb_angle = -150  # Fully relaxed, splayed out
        thumb_f = 0.0

    # Pass the customized thumb posture and the standard index flex
    draw_finger(mcp["thumb"], thumb_angle, thumb_f, thumb_f, [40, 30, 25])
    draw_finger(mcp["index"], -105, index_flex, index_flex, [55, 35, 25])
    
    # Middle Finger: ESP32 pad 0 (proximal base), SteamVR Grip (distal tip)
    draw_finger(mcp["mid"],   -90, flex_data[0], grip_flex, [60, 40, 25])
    
    # Ring & Pinky: ESP32 UDP data
    draw_finger(mcp["ring"],  -75, flex_data[2], flex_data[3], [55, 35, 25])
    draw_finger(mcp["pinky"], -60, flex_data[4], flex_data[5], [40, 25, 20])

last_packet_id = 0 
last_ping_time = 0
last_flex_data = [0.0] * 6 # Store last known good ESP32 data

def update_gui():
    global last_packet_id, last_ping_time, last_flex_data
    data = None
    
    # 1. ESP32 Auto-Discovery Broadcast
    if last_packet_id == 0 and time.time() - last_ping_time > 1.0:
        try:
            # Ensure you change this to your actual subnet broadcast IP if 255 fails
    ...
    # 2. Drain UDP Buffer for Capacitive Data
    try:
        while True:
    ...
    # 4. Render Frame (happens at ~100Hz regardless of whether new UDP data arrived)
    draw_skeleton(vr_flex, last_flex_data)
